...
build_user_list() {
    ...
    if [[ -n "$USERS_FILE" ]]; then
        ...
        log_error "Must specify users via --users-file, --count, or --users"
        exit 1
    fi
...
create_user_namespace() {
    local username="$1"
    local namespace="${NAMESPACE_PREFIX}-${username}"
...
spec:
  hard:
    ...
    count/devworkspaces.workspace.devfile.io: "1"
EOF
...
generate_access_urls() {
    ...
    if oc get route devspaces -n openshift-devspaces >/dev/null 2>&1; then
        ...
        devspaces_base_url="https://devspaces.apps.${cluster_domain}"
    fi
    ...
    for username in "${users[@]}"; do
        ...
        echo "  Namespace: ${NAMESPACE_PREFIX}-${username}"
        echo ""
    done
